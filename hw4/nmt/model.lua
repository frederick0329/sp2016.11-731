...
function model.buildEncoder(attMethod)
    ...
    protos.wordEmbed = nn.LookupTableMaskZero(nSrcWords, hiddenSize)
...
function model.buildDecoder()
    local protos = {}
    protos.lstm = LSTM.lstm(hiddenSize, hiddenSize)
    protos.wordEmbed = nn.LookupTableMaskZero(nTgtWords, hiddenSize)
    protos.sample = nn.Sequential():add(nn.SelectTable(2)):add(nn.Linear(hiddenSize, nTgtWords)):add(nn.LogSoftMax())
