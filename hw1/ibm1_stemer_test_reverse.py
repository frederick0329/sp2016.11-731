...
for (n, (f,e)) in enumerate(bitext):
    ...
    for idx, f_i in enumerate(f):
        temp_f[idx] = f_stemmer.stem(f_i)
    for idx, e_i in enumerate(e):
        temp_e[idx] = e_stemmer.stem(e_i)
    bitext2.append([temp_e, temp_f])
bitext=bitext2
...
p_e_f = pickle.load( open( "pretrain_p_f_e.prob", "rb" ) )
...
for f in fe_occur.keys():
    ...
    for e in fe_occur[f]:
        try:
            ...
            p_ef[f][e] = 0.00001/len(fe_occur[f])
